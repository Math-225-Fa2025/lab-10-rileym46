...
ggplot(data = evals, aes(x = score)) +
  geom_histogram()
```
`We can see that there is a clear left skew. This suggests that most courses are rated higher, with the max around 4.5 out of 5. This is what I expected to see. I feel like most courses are well done.`
...
ggplot(data = evals, aes(x = score, y = bty_avg))+
  geom_point()+
  labs(
    title = "Score and Beauty Average",
    x = "Score",
    y = "Beauty Average"
  )
```

`We can see a small amount of correlation, though it isn't too clear. It looks like most professors with a lower score rating also have a lower beuaty average.`
...
`Using jitter really affects the scatterplot. It makes it a lot more clear to where each point truly lands, making the data distribution more specific. The original scatterplot was too organized.`
...
```
`Score = 3.88 + 0.0666*bty_avg. For each point higher rated for bty_avg, expect 0.0666 points higher for score. Scores that have a 0 beauty rating are expected to be 3.88 points/stars.`
...
```{r bty-avg-regression, eval = TRUE}
...
`As the beauty average increases, the score also increases by 0.0666 points, on average.`
...
`When the beauty average is 0, the expected score is 3.88.`
...
```
`This is the percentage of variability in the response variable, score. 0.03502226 is a very small percentage. That means that there isn't much variability in the score and that this is not the best fit for this data.`
...
glimpse(score_gender_fit)
```
`Score = 4.093 + 0.142 * gender. This means that for every time a professor is male, his score is expected to increase by 0.142 points. For every time the professor is male, their score is expected to be 4.093.`

10. What is the equation of the line corresponding to male professors?
    What is it for female professors?

`Male: Score = 4.093 + 0.142 * gender. `
`Female: Score = `
...
```{r score-rank-fit, eval = TRUE}
score_rank_fit <- linear_reg() %>%
  set_engine("lm") %>%
  fit(score ~ rank, data = evals) 
tidy(score_rank_fit)
glimpse(score_rank_fit)

```
`Score = 4.284 - 0.130 * ranktenure track. For each tenured professor on track, their score is estimated to decrease by .130 points. For each professor without tenure, they are expected to have 4.284 stars.`
`Score = 4.284 - 0.145 * ranktenured. For each tenured professor, their score is estimated to decrease by 0.145 points.`
...
```{r score-rank-relevel-fit, eval = TRUE}
score_rank_relevel_fit <- linear_reg() %>%
  set_engine("lm") %>%
  fit(score ~ rank_relevel, data = evals)
tidy(score_rank_relevel_fit)
glimpse(score_rank_relevel_fit)

glance(score_rank_relevel_fit)$r.squared
```
`Score = 4.155 + 0.130*teaching. Professors ranked with teaching are expected to have, on average, 0.130 points higher than professors that are tenure track. Professors that are tenure track are expected to have 4.155 stars, on average.`
`Score = 4.155 - 0.015*tenured. Professors ranked with tenured are expected to have, on average, 0.015 points fewer than professors that are tenure track.`
`The $r.squared of this model is 0.0116, meaning there is a very weak relationship between the two variables.`
...
```{r score-tenure-eligible-fit, eval = TRUE}
score_tenure_eligible_fit <- linear_reg() %>%
  set_engine("lm") %>%
  fit(score ~ tenure_eligible, data = evals)
tidy(score_tenure_eligible_fit)
glimpse(score_tenure_eligible_fit)

glance(score_tenure_eligible_fit)$r.squared
```
`Score = 4.284 - 0.141*tenure_eligibleyes. For every professor that is eligible for tenure, their score is expected to, on average, decrease by 0.141 points. Professors that are not eligible are expected to have, on average, a score of 4.284.`
`The $r.squared score for this is 0.0115, meaning that there is a low percentage of variability in the score.`
